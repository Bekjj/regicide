...
export default function Home() {
  // --- СОСТОЯНИЯ ---
  const [view, setView] = useState('menu'); // menu, boss_select, game, shop, history, settings
  ...
  // Имитация доски 8x8 (0 - пусто, 1 - игрок (светлые), 2 - босс (темные))
  ...
  const calculateElo = (playerRating, opponentRating, isWin) => {
    const K = 32;
  ...
  const handleCellClick = (r, c) => {
    // 1. Если кликнули на свою фигуру - выделяем её
    if (board[r][c] === 1) {
      setSelectedPiece({r, c});
      setBossComment("Думаешь, это хорошая идея?");
    } 
    // 2. Если фигура выделена и кликнули на пустую темную клетку - ходим
    else if (selectedPiece && board[r][c] === 0 && (r + c) % 2 === 1) {
      const newBoard = [...board];
      newBoard[selectedPiece.r][selectedPiece.c] = 0; // убираем со старого места
      newBoard[r][c] = 1; // ставим на новое
      setBoard(newBoard);
      setSelectedPiece(null);
      setBossComment("Хм... Мой ход.");
      
      // Имитация ответа босса (через 1 секунду)
      setTimeout(() => {
  ...
  return (
    <div className={`min-h-screen font-serif flex flex-col transition-colors duration-500 ${activeSkin === 'skin_neon' ? 'bg-black text-cyan-400' : 'bg-[#1a1614] text-[#e6d5c3]'}`}>
      ...
      <nav className="w-full p-4 border-b border-[#3a2e26] bg-[#120f0e] shadow-xl flex justify-between items-center">
        <h1 
          className="text-2xl font-bold tracking-widest uppercase cursor-pointer hover:text-amber-500 transition"
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8">
        ...
        {view === 'menu' && (
          <div className="w-full max-w-md flex flex-col gap-4 animate-fade-in">
            ...
            <button onClick={() => setView('boss_select')} className="w-full py-4 bg-[#2a221d] hover:bg-[#3a2e26] border border-[#4a3b32] rounded-lg text-xl font-bold transition transform hover:scale-105 shadow-lg">
              ⚔️ Играть против Босса
            </button>
            <button onClick={() => setView('history')} className="w-full py-4 bg-[#1a1614] hover:bg-[#2a221d] border border-[#3a2e26] rounded-lg text-lg transition shadow-md">
              📜 Архив партий
            </button>
            <button onClick={() => setView('shop')} className="w-full py-4 bg-[#1a1614] hover:bg-[#2a221d] border border-[#3a2e26] rounded-lg text-lg transition shadow-md">
              💎 Магазин скинов
            </button>
            <button onClick={() => setView('settings')} className="w-full py-4 bg-[#1a1614] hover:bg-[#2a221d] border border-[#3a2e26] rounded-lg text-lg transition shadow-md">
              ⚙️ Настройки
            </button>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
              {BOSSES.map((boss) => (
                <div key={boss.id} className="p-6 border border-[#3a2e26] rounded-xl bg-[#1e1916] hover:bg-[#2a221d] cursor-pointer transition transform hover:-translate-y-2 hover:shadow-[0_0_15px_rgba(217,119,6,0.2)] hover:border-amber-700/50 flex flex-col h-full" onClick={() => startGame(boss)}>
                  ...
                  <div className="mt-auto pt-4 border-t border-[#3a2e26]">
                    <span className="text-yellow-600/80 font-bold text-sm">Победа: +{boss.coins} 💰</span>
                  </div>
                </div>
              ))}
            </div>
            
            <button onClick={() => setView('menu')} className="mt-4 px-8 py-2 border border-[#4a3b32] rounded hover:bg-[#2a221d] transition">
              Назад в меню
            </button>
          </div>
        )}
        ...
        {view === 'game' && selectedBoss && (
          <div className="flex flex-col lg:flex-row gap-8 w-full max-w-5xl items-center lg:items-start justify-center animate-slide-up">
            ...
            <div className="w-full lg:w-1/3 p-6 bg-[#1e1916] border border-[#3a2e26] rounded-xl flex flex-col items-center text-center shadow-2xl relative">
              ...
              {/* Чат босса */}
              <div className="relative bg-[#120f0e] p-4 rounded-lg border border-[#3a2e26] w-full mb-8 italic min-h-[80px] flex items-center justify-center">
                "{bossComment}"
              </div>
              
              <div className="w-full space-y-3 mb-6">
                 <p className="text-xs opacity-40 text-left">💡 Чтобы походить: кликни на свою (светлую) шашку, затем на пустую темную клетку.</p>
              </div>

              {/* Кнопки симуляции для хакатона */}
                    ...
                    return (
                      <div 
                        ...
                        className={`
                          ...
                          ${isDarkCell ? 'bg-[#4a3525] hover:bg-[#5a422e]' : 'bg-[#d0b49f] cursor-default'}
                          ${selectedPiece?.r === r && selectedPiece?.c === c ? 'ring-inset ring-4 ring-amber-400 bg-[#7a5530]' : ''}
                        `}
                      >
        ...
        {view === 'history' && (
          <div className="w-full max-w-2xl animate-fade-in">
            ...
            <div className="bg-[#1e1916] rounded-xl border border-[#3a2e26] p-4 min-h-[300px]">
              {matchHistory.length === 0 ? (
                <div className="h-full flex items-center justify-center opacity-50 italic">
                  Вы еще не сыграли ни одной партии.
                </div>
              ) : (
                <div className="space-y-3">
                  {matchHistory.map((match, idx) => (
                    <div key={idx} className="flex justify-between items-center p-4 bg-[#120f0e] border border-[#2a221d] rounded">
                      <div>
                        <p className="font-bold">{match.boss}</p>
                        <p className="text-xs opacity-50">{match.date}</p>
                      </div>
                      <div className="text-right">
                        <p className={match.result === 'Победа' ? 'text-green-500 font-bold' : 'text-red-500 font-bold'}>{match.result}</p>
                        <p className="text-sm opacity-60 font-mono">{match.eloChange} ELO</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <button onClick={() => setView('menu')} className="mt-6 w-full py-3 border border-[#4a3b32] rounded hover:bg-[#2a221d] transition">
              Назад в меню
            </button>
          </div>
        )}
        ...
        {view === 'shop' && (
          <div className="w-full max-w-2xl animate-fade-in">
                ...
                <div key={item.id} className="flex justify-between items-center p-5 bg-[#1e1916] border border-[#3a2e26] rounded-xl">
                  ...
                  {item.unlocked ? (
                    <button 
                      ...
                      className={`px-6 py-2 rounded font-bold transition ${activeSkin === item.id ? 'bg-amber-700/20 text-amber-500 border border-amber-700/50' : 'bg-[#2a221d] hover:bg-[#3a2e26] border border-[#4a3b32]'}`}
                    >
                    ...
                    <button 
                      ...
                      className={`px-6 py-2 rounded font-bold flex items-center gap-2 transition ${coins >= item.price ? 'bg-yellow-900/30 text-yellow-500 hover:bg-yellow-900/50 border border-yellow-700/50' : 'bg-[#120f0e] text-stone-600 cursor-not-allowed border border-[#2a221d]'}`}
                    >
            ...
            <button onClick={() => setView('menu')} className="mt-8 w-full py-3 border border-[#4a3b32] rounded hover:bg-[#2a221d] transition">
              Назад в меню
            </button>
          </div>
        )}
        ...
        {view === 'settings' && (
          <div className="w-full max-w-md animate-fade-in">
            ...
            <div className="bg-[#1e1916] p-6 rounded-xl border border-[#3a2e26] space-y-6 text-center opacity-70">
              ...
              <p className="text-sm">В будущем здесь будет привязка аккаунта, настройка звука и интеграция Stripe для доната.</p>
            </div>
            <button onClick={() => setView('menu')} className="mt-8 w-full py-3 border border-[#4a3b32] rounded hover:bg-[#2a221d] transition">
              Назад в меню
            </button>
      ...
      {/* CSS Анимации (встроенные для простоты) */}
      <style jsx global>{`
